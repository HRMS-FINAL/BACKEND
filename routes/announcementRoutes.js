// routes/announcementRoutes.js — Announcement CRUD
//
// HRMS and the mobile backend share the same `announcements` collection
// (unified DB). When HR creates an announcement here we write ONE document
// with both `description` (HRMS UI field) and `body` (mobile UI field) so
// either app can render it without re-translating. There is NO mirror call
// to the mobile backend — that would write a second document to the same
// shared collection and produce duplicates everywhere.
...
router.put('/:id', async (req, res) => {
  try {
    ...
    // Unified DB: no mirror call. Keep the `body` field in sync with
    // `description` so the mobile reader sees the updated text.
    if (typeof update.description === 'string' && announcement.body !== update.description) {
      try {
        announcement.set('body', update.description);
        await announcement.save();
      } catch (e) {
        console.warn('[announcement] could not mirror body field:', e.message);
      }
    }
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    // Unified DB: soft-delete on the local doc is enough — the mobile
    // app's GET filters by `isActive: true` so the row disappears there too.
...
// ─── One-shot cleanup of mirror duplicates ────────────────────────────
// Before the unified-DB fix, every HRMS-posted announcement also got a
// mirror copy via /api/announcement/admin which stamped an `externalId`
// field. Now that we no longer mirror, those mirror copies are pure
// duplicates. They're identifiable by `externalId` being non-null.
//
// GET  /api/announcements/cleanup-duplicates  → returns count (dry run)
// POST /api/announcements/cleanup-duplicates  → soft-deletes them all
router.get('/cleanup-duplicates', async (req, res) => {
  try {
    const filter = { externalId: { $exists: true, $nin: [null, ''] }, isActive: true };
    const count = await Announcement.countDocuments(filter);
    res.json({ success: true, mirrorDuplicates: count, action: 'POST this URL to soft-delete them' });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.post('/cleanup-duplicates', async (req, res) => {
  try {
    const filter = { externalId: { $exists: true, $nin: [null, ''] }, isActive: true };
    const result = await Announcement.updateMany(filter, { isActive: false });
    res.json({
      success: true,
      deactivated: result.modifiedCount ?? result.nModified ?? 0,
      message: 'Mirror duplicates soft-deleted. Refresh both apps to see the clean list.',
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
